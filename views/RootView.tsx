...
const RootView: React.FC<Props> = (props) => {
  ...
  const setUpWallet = useCallback(
    async (isRestoringOldWallet: boolean) => {
      ...
      dispatch(
        addExternalAddress(
          new AddressLookup(0, firstExternal, zeroOrMinusOne, false),
        ),
      );
      ...
      if (isRestoringOldWallet) {
        for (var i = 0; i < 20; i++) {
          ...
          dispatch(
            addExternalAddress(
              new AddressLookup(i + 1, external, zeroOrMinusOne, true),
            ),
          );
        }
      }
      ...
      dispatch(
        addInternalAddress(
          new AddressLookup(0, firstInternal, zeroOrMinusOne, false),
        ),
      );
      ...
      if (isRestoringOldWallet) {
        for (var i = 0; i < 20; i++) {
          ...
          dispatch(
            addInternalAddress(
              new AddressLookup(i + 1, internal, zeroOrMinusOne, true),
            ),
          );
        }
      }
  ...
  const generateSeed = async () => {
    ...
    await RNSecureKeyStore.set('WALLET_SEED', words, {
  ...
  }, [isActive, isNewWallet, isWalletRestoring, setUpWallet, sync]);

  useEffect(() => {
    handleEffect();
  }, [isActive, isWalletRestoring, isNewWallet, handleEffect]);
  ...
  return (
    <View style={{flex: 1}}>
      {(isWalletRestoring || isNewWallet) && (
        <Loader
          title={
            isNewWallet ? 'Creating wallet' : getTranslated(language).restoring
          }
          subTitle="This might take a second"
        />
      )}
      {isActive && !isWalletRestoring && !isNewWallet && (
        <Tab.Navigator
          tabBarOptions={{
            ...
            inactiveBackgroundColor: '#090C14',
            activeTintColor: '#F7931A',
            activeBackgroundColor: '#090C14',
            style: {backgroundColor: '#090C14', borderTopColor: '#1F232E'},
          }}>
          <Tab.Screen
            name="Overview"
            component={Overview}
            options={{
              tabBarLabel: getTranslated(language).overview,
              tabBarIcon: (tabProps) => {
                return tabProps.focused ? (
                  <Image
                    style={styles.tabIcon}
                    source={require('../assets/images/collection-focus.png')}
                  />
                ) : (
                  <Image
                    style={styles.tabIcon}
          ...
          <Tab.Screen
            ...
            component={shell}
            options={{
              tabBarLabel: getTranslated(language).send,
              tabBarButton: (values) => (
                <TouchableOpacity
                  {...values}
                  onPress={() => props.navigation.push('Send')}
                />
              ),
              tabBarIcon: (tabProps) => {
                return tabProps.focused ? (
                  <Image
                    style={styles.tabIcon}
                    source={require('../assets/images/send-focus.png')}
                  />
                ) : (
                  <Image
                    style={styles.tabIcon}
          ...
          <Tab.Screen
            ...
            component={Receive}
            options={{
              tabBarLabel: getTranslated(language).receive,
              tabBarIcon: (tabProps) => {
                return tabProps.focused ? (
                  <Image
                    style={styles.tabIcon}
                    source={require('../assets/images/receive-focus.png')}
                  />
                ) : (
                  <Image
                    style={styles.tabIcon}
          ...
          <Tab.Screen
            ...
            component={Settings}
            options={{
              tabBarLabel: getTranslated(language).settings,
              tabBarIcon: (tabProps) => {
                return tabProps.focused ? (
                  <Image
                    style={styles.tabIcon}
                    source={require('../assets/images/gear-focus.png')}
                  />
                ) : (
                  <Image
                    style={styles.tabIcon}
      ...
      {!isActive && !isNewWallet && !isWalletRestoring && (
        <View style={{flex: 1}}>
          <Header
            screen={getTranslated(language).getting_started}
            currentLanguage={getLanguageBigName(language)}
            action={() => {
              props.navigation.navigate('PickerView', {
              ...
              <View style={styles.container}>
                <View style={styles.onboard}>
                  <Text style={styles.headingText}>
                    {getTranslated(language).getting_started}
                  </Text>
                  <View style={styles.buttonContainer}>
                    <LinearGradient
                      ...
                      colors={['#1f232e', '#13161f']}>
                      <TouchableOpacity
                        style={styles.onboardButton}
                        onPress={() =>
                          props.navigation.navigate('CreateStepOne')
                        }>
                        <View style={styles.buttonContent}>
                          <Image
                            style={styles.onboardIcon}
                            source={require('../assets/images/create.png')}
                          />
                          <View style={{marginLeft: 20}}>
                            <Text style={styles.onboardButtonText}>
                              {getTranslated(language).create_new}
                            </Text>
                            <Text style={styles.onboardButtonSubText}>
                              {getTranslated(language).create_subtext}
                            </Text>
                          </View>
                        </View>
                      </TouchableOpacity>
                    ...
                    <LinearGradient
                      ...
                      <TouchableOpacity
                        ...
                        <View style={styles.buttonContent}>
                          ...
                          <View style={{marginLeft: 20}}>
                            <Text style={styles.onboardButtonText}>
                              {getTranslated(language).restore_existing}
                            </Text>
                            <Text style={styles.onboardButtonSubText}>
                              {getTranslated(language).restore_subtext}
                            </Text>
                          </View>
                        </View>
                      </TouchableOpacity>
